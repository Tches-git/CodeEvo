...
class RegressionEvaluator:
    ...
    def run(self, prompt: str, cases: List[dict]) -> Dict[str, Any]:
        ...
        for case in cases:
            ...
            case_results[-1]["latency_ms"] = round(
                (time.monotonic() - case_started) * 1000, 4
            )
            case_results[-1]["usage"] = usage_delta(usage_before, usage_after)
        ...
        return {
            ...
            "resource_usage": summarize_resources(case_results),
            "errors": errors,
...
class EvolutionEngine:
    ...
    def __init__(
        ...
        max_latency_growth_ratio: float = 0.25,
        max_token_growth_ratio: float = 0.20,
        max_cost_growth_ratio: float = 0.20,
        ...
        self.evaluation_policy = EvaluationPolicy(
            minimum_quality_improvement=min_improvement,
            maximum_quality_regression=max_metric_regression,
            maximum_latency_growth_ratio=max_latency_growth_ratio,
            maximum_token_growth_ratio=max_token_growth_ratio,
            maximum_cost_growth_ratio=max_cost_growth_ratio,
        )
    ...
    def status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "evaluation_policy": self.evaluation_policy.evaluate(
                self._empty_metrics(0), self._empty_metrics(0),
                require_improvement=False, protected_quality_metrics=(),
            )["policy"],
    ...
    def _propose(
        ...
        if not safety["safety_passed"] or safety["completeness"] < 1.0:
            ...
            validation_policy = self.evaluation_policy.evaluate(
                baseline_metrics, candidate_metrics
            )
            holdout_policy = self.evaluation_policy.evaluate(
                baseline_holdout, candidate_holdout, require_improvement=False
            )
            validation_safe = validation_safe and validation_policy["passed"]
            holdout_safe = holdout_safe and holdout_policy["passed"]
            gates.update({
    ...
    def _empty_metrics(case_count: int) -> Dict[str, Any]:
        return {
            ...
            "successful_cases": 0, "success_rate": 0.0,
            "resource_usage": summarize_resources([]),
            "errors": [], "case_results": [],
        }

...
class Reproduction:
    ...
    method: str
    evidence: str
    repository_verified: Optional[bool] = None
        ...
        if workspace is not None:
            ...
            try:
                context = workspace.read_context(
                    finding.path, finding.line, context_lines
                )
                repository_evidence = context.get("target_content", "").strip()
                repository_verified = bool(
                    repository_evidence
                    and finding.evidence
                    and finding.evidence.strip() in repository_evidence
                )
                if repository_verified:
                    ...
                    reference["excerpt"] = repository_evidence[:240]
                    evidence_refs.append(reference)
                    if reference not in finding.context_evidence:
                        finding.context_evidence.append(reference)
            except (OSError, UnicodeError, ValueError):
                repository_verified = False
        elif repository_context_required:
            repository_verified = False
        return Reproduction(
            finding_key(finding), reproducible,
            (
                "changed-line and repository-context evidence check"
                if repository_verified is not None
                else "independent changed-line evidence check"
            ),
            (
                repository_evidence[:240]
                if repository_verified
                else line.strip()[:240] if reproducible
                else "No independently matching changed-line evidence."
            ),
            repository_verified,
            evidence_refs,
        )
...
class VerifierAgent:
    ...
    def verify(
        ...
        if (
            ...
            and reproduction.repository_verified is False
        ):
            reasons.append(
                "high-risk finding is not bound to matching code read from the repository"
            )
        if not fix_ready:
...
class MultiAgentCoordinator(Reviewer):
    ...
    def __init__(
        ...
        workspace_resolver=None, repository_context_lines: int = 4,
    ):
        self.agents = agents
        ...
        self.repository_context_lines = max(1, int(repository_context_lines))
        self.agent_loop_enabled = bool(agent_loop_enabled)
    ...
    def review_with_context(
        ...
        workspace = (
            self.workspace_resolver.resolve(repository)
            if self.workspace_resolver is not None else None
        )
        state: CollaborationState = {
            ...
            "repository_context_required": bool(
                ...
                and self.workspace_resolver.enabled
            ),
        }
    ...
    def _agent_tools(
        ...
        session = WorkspaceSession(workspace) if workspace is not None else None
        if workspace is not None:
            tools.extend([
                AgentTool(
                    "list_repository_files",
                    "List readable files from the configured read-only repository checkout.",
                    {
                        "type": "object",
                        "properties": {
                ...
                AgentTool(
                    ...
                    "Read at most 200 lines from a repository file and return a verifiable evidence ID.",
                    {
                        "type": "object",
                        "properties": {
                            "path": {"type": "string"},
                ...
                AgentTool(
                    "search_repository",
                    "Search readable repository files for a literal case-insensitive fragment.",
                    {
                        "type": "object",
                        "properties": {
                            "query": {"type": "string"},
                ...
                AgentTool(
                    ...
                    "Report Tree-sitter parser, indexed languages and truncation state.",
                    {
                        "type": "object", "properties": {},
                        "additionalProperties": False,
                    },
                ...
                AgentTool(
                    "find_symbol",
                    "Find language-aware symbol definitions using the Tree-sitter index.",
                    {
                        "type": "object",
                        "properties": {
                            "query": {"type": "string"},
                    ...
                    workspace.index.find_symbols,
                ),
                AgentTool(
                    "find_references",
                    "Find language-aware identifier references, optionally within one file.",
                    {
                        "type": "object",
                        "properties": {
                            "symbol": {"type": "string"},
                    ...
                    workspace.index.find_references,
                ),
                AgentTool(
                    "find_callers",
                    "Find functions or methods that call a symbol using Tree-sitter call nodes.",
                    {
                        "type": "object",
                        "properties": {
                            "symbol": {"type": "string"},
                    ...
                    workspace.index.find_callers,
                ),
            ])
        return ToolRegistry(tools), session
    ...
    def _run_agent_loop(
        ...
        tools, workspace_session = self._agent_tools(state, assignment)
        ...
        if workspace_session is not None:
            for finding in findings:
                for reference in workspace_session.evidence_for(
    ...
    def _invoke_agent(
        ...
        for attempt in range(1, self.agent_retries + 2):
            ...
            try:
                loop_stepper = (
                    getattr(agent, "agent_step", None)
                    if self.agent_loop_enabled else None
                )
    ...
    def _evidence_node(self, state: CollaborationState) -> Dict[str, Any]:
        ...
        for finding in state["specialist_findings"]:
            reproduction = self.evidence_agent.reproduce(
                finding,
                state["parsed"],
                workspace=state.get("workspace"),
                repository_context_required=state.get(
                    "repository_context_required", False
                ),
                context_lines=self.repository_context_lines,
            )
    ...
    def _make_summary(self, state: CollaborationState) -> dict:
        ...
        return {
            ...
            "repository_context": {
                "required": state.get("repository_context_required", False),
                "available": state.get("workspace") is not None,
                "index": (
                    state["workspace"].index.summary_status()
                    if state.get("workspace") is not None else None
                ),
                "evidence_bound_findings": sum(
                    1 for item in state.get("verified", [])
                    if item.context_evidence
                ),
                "evidence_rejections": sum(
                    1 for item in decisions.values()
                    if any(
                        "not bound to matching code" in reason

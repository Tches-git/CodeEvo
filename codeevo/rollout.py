...
class RoutingPolicyEvaluator:
    """Offline release gate for routing policies before shadow/canary rollout."""

    def __init__(self, policy: Optional[EvaluationPolicy] = None):
        self.policy = policy or EvaluationPolicy(minimum_quality_improvement=0.0)
    ...
    def evaluate(
        ...
        result = self.policy.evaluate(
            baseline, candidate, require_improvement=require_improvement,
        ...
        return {
            "schema_version": 1,
            "strategy": "offline-routing-replay",
            "decision": "eligible-for-shadow" if result["passed"] else "rejected",
            **result,
        }


class ReleaseManager:
    def __init__(self, store, evaluation_policy: Optional[EvaluationPolicy] = None):
        self.store = store
        self.routing_evaluator = RoutingPolicyEvaluator(evaluation_policy)

    def evaluate_candidate(
        self, baseline: Dict[str, Any], candidate: Dict[str, Any],
        require_improvement: bool = False,
    ) -> Dict[str, Any]:
        return self.routing_evaluator.evaluate(
            baseline, candidate, require_improvement
        )
    ...
    def configure(self, tenant_id: str, skill_name: str, config: Dict[str, object]) -> dict:
        ...
        if canary or shadow:
            evidence = config.get("offline_evaluation")
            if not isinstance(evidence, dict):
                raise ValueError(
                    "a passing offline_evaluation is required before shadow or canary"
                )
            baseline = evidence.get("baseline")
            candidate = evidence.get("candidate")
            if not isinstance(baseline, dict) or not isinstance(candidate, dict):
                raise ValueError("offline_evaluation requires baseline and candidate metrics")
            evaluation = self.evaluate_candidate(
                baseline, candidate, bool(evidence.get("require_improvement", False))
            )
            if not evaluation["passed"]:
                raise ValueError("routing candidate failed the offline evaluation gate")
            config = {
                **config,
                "offline_evaluation": {

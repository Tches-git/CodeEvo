...
class AuthManager:
    def __init__(
        ...
        if bootstrap_username and bootstrap_password:
            self.store.create_user(
                str(uuid.uuid5(uuid.NAMESPACE_URL, "codeevo:" + bootstrap_username)),
    ...
    def create_installation_state(
        ...
        """Create a short-lived state token for the GitHub App install redirect."""
        now = int(time.time())
        return self._encode({
            ...
            "role": principal.role,
            "iat": now,
            "exp": now + min(max(1, ttl_seconds), self.ttl_seconds),
        })

    def verify_installation_state(self, state: str) -> Principal:
        """Validate install state and re-check the user's current membership."""
        payload = self._decode(state)
        if payload.get("kind") != "github-installation":
            raise PermissionError("invalid GitHub installation state")
        user = self.store.get_user(str(payload.get("username", "")))
        if not user or not user["active"] or str(user["id"]) != str(payload.get("sub", "")):
            raise PermissionError("GitHub installation state is no longer authorized")
        tenant_id = str(payload.get("tenant", ""))
        memberships = {item["tenant_id"]: item["role"] for item in user["memberships"]}
        role = memberships.get(tenant_id, "")
        principal = Principal(str(user["id"]), str(user["username"]), tenant_id, role)

"""Checkpointed review workflow powered by CodeEvo's own runtime."""
import threading
...
class ReviewHarness:
    ...
    def _reviewing(self, state: RuntimeState) -> Dict[str, Any]:
        ...
        if repository_context.get("required") and not repository_context.get("available"):
            summary += (
                " Repository context was unavailable; high-risk claims were "
                "rejected by the evidence gate."
            )
        elif repository_context.get("evidence_rejections"):
            summary += (
                " %d high-risk claim(s) were rejected because the repository "
                "checkout did not provide matching evidence."
                % repository_context["evidence_rejections"]
            )
        report = ReviewReport(

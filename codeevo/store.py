...
class TaskStore:
        ...
        with self._connect() as conn:
            ...
            conn.execute(
                """CREATE TABLE IF NOT EXISTS annotation_submissions (
                    ...
                    UNIQUE(case_id, annotator_id),
                    FOREIGN KEY(case_id) REFERENCES annotation_cases(id)
                )"""
            )
            conn.execute(
            ...
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_annotation_cases_queue "
                "ON annotation_cases(tenant_id, status, split, created_at)"
            )
            conn.execute(
                "CREATE INDEX IF NOT EXISTS idx_annotation_submissions_case "
                "ON annotation_submissions(tenant_id, case_id, submitted_at)"
            )
    ...
    def find_annotation_case(
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM annotation_cases WHERE tenant_id=? AND lower(repository)=lower(?) "
                "AND pull_request=?", (tenant_id, repository, pull_request),
            ).fetchone()
    ...
    def get_annotation_case(
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM annotation_cases WHERE id=? AND tenant_id=?",
                (case_id, tenant_id),
            ).fetchone()
        return self._annotation_case(row) if row else None
    ...
    def update_annotation_case_status(
        ...
        with self._lock, self._connect() as conn:
            ...
            placeholders = ",".join("?" for _item in allowed)
            cursor = conn.execute(
                "UPDATE annotation_cases SET status=?,updated_at=?,"
                "exported_at=COALESCE(?,exported_at) WHERE id=? AND tenant_id=? "
                "AND status IN (" + placeholders + ")",
                (status, updated_at, exported_at, case_id, tenant_id, *allowed),
            )
            if cursor.rowcount == 1:
                return
            row = conn.execute(
                "SELECT status FROM annotation_cases WHERE id=? AND tenant_id=?",
                (case_id, tenant_id),
            ).fetchone()
            if not row:
            ...
            if row["status"] == status:
                return
            # A concurrent second submission may already have advanced the state.
    ...
    def create_annotation_submission(self, submission: Dict[str, Any]) -> Dict[str, Any]:
        with self._lock, self._connect() as conn:
            case = conn.execute(
                "SELECT required_reviewers FROM annotation_cases WHERE id=? AND tenant_id=?",
                (submission["case_id"], submission["tenant_id"]),
            ).fetchone()
            if not case:
                raise ValueError("annotation case not found")
            count = conn.execute(
                "SELECT COUNT(*) AS n FROM annotation_submissions WHERE case_id=? AND tenant_id=?",
                (submission["case_id"], submission["tenant_id"]),
            ).fetchone()["n"]
    ...
    def list_annotation_submissions(self, case_id: str, tenant_id: str) -> list:
        with self._connect() as conn:
            rows = conn.execute(
                "SELECT * FROM annotation_submissions WHERE case_id=? AND tenant_id=? "
        ...
        with self._lock, self._connect() as conn:
            try:
                conn.execute(
                    ...
                    "adjudicator,verdict,findings_json,rationale,created_at) VALUES (?,?,?,?,?,?,?,?,?)",
                    (
    ...
    def get_annotation_adjudication(
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM annotation_adjudications WHERE case_id=? AND tenant_id=?",
                (case_id, tenant_id),
            ).fetchone()
        if not row:
            return None
        ...
        with self._lock, self._connect() as conn:
            try:
                conn.execute(
                    ...
                    "dataset_json,created_by,created_at) VALUES (?,?,?,?,?,?,?,?)",
                    (
    ...
    def list_annotation_exports(self, tenant_id: str, limit: int = 100) -> list:
        with self._connect() as conn:
            rows = conn.execute(
                ...
                "FROM annotation_exports WHERE tenant_id=? ORDER BY created_at DESC LIMIT ?",
                (tenant_id, max(1, min(limit, 500))),
            ).fetchall()
        values = []
        for row in rows:
    ...
    def get_annotation_export(
        ...
        with self._connect() as conn:
            row = conn.execute(
                "SELECT * FROM annotation_exports WHERE id=? AND tenant_id=?",
                (export_id, tenant_id),
            ).fetchone()
        if not row:
            return None

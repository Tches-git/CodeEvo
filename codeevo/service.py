...
from .workspace import RepositoryWorkspaceResolver
...
class ReviewService:
    def __init__(self, settings: Settings):
        ...
        self.annotations = AnnotationService(
            self.store, self.github, settings.max_diff_bytes
        )
        ...
        self.releases = ReleaseManager(self.store, EvaluationPolicy(
            ...
            maximum_latency_growth_ratio=settings.eval_max_latency_growth_ratio,
            maximum_token_growth_ratio=settings.eval_max_token_growth_ratio,
            maximum_cost_growth_ratio=settings.eval_max_cost_growth_ratio,
        ))
        ...
        self.evolution = EvolutionEngine(
            ...
            max_latency_growth_ratio=settings.eval_max_latency_growth_ratio,
            max_token_growth_ratio=settings.eval_max_token_growth_ratio,
            max_cost_growth_ratio=settings.eval_max_cost_growth_ratio,
        )
    ...
    def _build_llm_reviewer(self, prompt: str = "") -> OpenAICompatibleReviewer:
        ...
        return OpenAICompatibleReviewer(
            ...
            input_cost_per_million=self.llm_config.get("input_cost_per_million"),
    ...
    def _build_coordinator(self, reviewers: list) -> MultiAgentCoordinator:
        return MultiAgentCoordinator(
            ...
            repository_context_lines=self.settings.repository_context_lines,
        )
    ...
    def repository_context_status(
        self, repository: str, tenant_id: str = "default", build_index: bool = False,
    ) -> dict:
        self._authorize_repository(tenant_id, repository)
        workspace = self.workspace_resolver.resolve(repository)
        index = None
        if workspace is not None:
            index = (
                workspace.index.status()
                if build_index else workspace.index.summary_status()
            )
        return {
            "enabled": self.workspace_resolver.enabled,
            "available": workspace is not None,
            "repository": repository,
            "mapping": "<repository-root>/<owner>/<repository>",
            "index": index,
        }
    ...
    def _process_queued(self, payload: Dict[str, Any]) -> None:
        ...
        if diff is None and payload.get("diff_url"):
            client = (
                self.github_client_for_installation(
                    payload.get("installation_id"), tenant_id
                )
    ...
    def github_client_for_installation(
        self, installation_id: Optional[int] = None, tenant_id: str = "",
    ) -> GitHubClient:
        if installation_id is None:
            return self.github
        if tenant_id and self.store.installation_tenant(installation_id) != tenant_id:
            raise PermissionError("GitHub installation is not authorized for this tenant")
    ...
    def create_fix(
        ...
        result = self.fixer.create_fix_commits(
            self.github_client_for_installation(installation_id, actual_tenant),
    ...
    def _authorize_repository(self, tenant_id: str, repository: str) -> None:
        # The unauthenticated, loopback-oriented development mode keeps the
        # zero-config demo flow. Authenticated deployments are deny-by-default.

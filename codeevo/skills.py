...
class SandboxedSkillReviewer(Reviewer):
    ...
    def review(self, diff, parsed) -> List[Finding]:
        ...
        with tempfile.TemporaryDirectory(prefix="codeevo-skill-") as workdir:
            env = {
...
class SkillRegistry:
    ...
    def reload(self) -> List[dict]:
        ...
        for entry in os.scandir(self.skills_dir):
            ...
            if not self.sandbox:
                raise ValueError("dynamic skills require CODEEVO_SKILL_SANDBOX=true")
            self.register(

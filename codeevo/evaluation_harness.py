...
def validate_case(case: dict, line_number: int = 0) -> None:
    ...
    scoring = case.get("scoring") or {"scope": "finding"}
    if not isinstance(scoring, dict):
        raise ValueError("%s scoring must be an object" % prefix)
    scope = str(scoring.get("scope", "finding"))
    if scope not in {"finding", "target-cwe"}:
        raise ValueError("%s has invalid scoring scope" % prefix)
    target_cwes = {
        str(value).upper() for value in (scoring.get("target_cwes") or [])
    }
    if scope == "target-cwe" and not target_cwes:
        raise ValueError("%s target-cwe scoring needs target_cwes" % prefix)
    added_locations = {
    ...
    for expected in case["expected_findings"]:
        ...
        if scope == "target-cwe" and str(expected["cwe"]).upper() not in target_cwes:
            raise ValueError("%s finding is outside target_cwes" % prefix)
    ...
    for expected_index, truth in enumerate(expected):
        ...
        for predicted_index, finding in enumerate(predicted):
            if RULE_TO_CWE.get(finding.rule_id, finding.rule_id).upper() != truth_cwe:
                continue
            if match_scope == "target-cwe":
                options.append((predicted_index, 0))
                continue
...
def one_to_one_match(
    ...
    edges = _candidate_edges(expected, predicted, line_tolerance, match_scope)
...
class EndToEndEvaluationHarness:
    ...
    def run(self, reviewer: Reviewer, cases: List[dict], name: str = "") -> Dict[str, Any]:
        ...
        case_results = []
        for case in cases:
            result = self.run_case(reviewer, case)
    ...
    def summarize(
        ...
        """Aggregate completed case results without re-running a reviewer."""
        if len(cases) != len(case_results):
            raise ValueError("case_results must contain exactly one result per case")
        expected_ids = [str(case["id"]) for case in cases]
        actual_ids = [str(result.get("id", "")) for result in case_results]
        if expected_ids != actual_ids:
            raise ValueError("case_results order must match the dataset case order")
        totals = self._empty_totals()
        for result in case_results:
            self._accumulate(totals, result)
        metrics = self.metrics_for(case_results)
        by_split = {}
        for split in ("train", "validation", "holdout"):
            selected = [item for item in case_results if item["split"] == split]
            by_split[split] = self.metrics_for(selected)
        source_kinds = sorted({
        ...
        return {
            ...
            "dataset": {
                ...
                "integrity": validate_dataset_integrity(cases),
            },
            "metrics": metrics,
            "by_split": by_split,
            "duration_seconds": round(
                ...
                else sum(float(item.get("latency_ms", 0.0)) for item in case_results) / 1000,
                4,
            ),
    ...
    @classmethod
    def metrics_for(cls, case_results: Iterable[dict]) -> Dict[str, Any]:
        """Aggregate any case-result subset using the canonical metric formulas."""
        results = list(case_results)
        totals = cls._empty_totals()
        for result in results:
            cls._accumulate(totals, result)
        metrics = cls._metrics(totals)
        metrics["resource_usage"] = summarize_resources(results)
        return metrics

    def run_case(self, reviewer: Reviewer, case: dict) -> Dict[str, Any]:
        """Run and score one case, including its isolated resource usage."""
        case_started = time.monotonic()
        usage_before = read_evaluation_usage(reviewer)
        expected = list(case["expected_findings"])
        scoring = case.get("scoring") or {"scope": "finding"}
        scoring_scope = str(scoring.get("scope", "finding"))
        target_cwes = {
            str(value).upper() for value in (scoring.get("target_cwes") or [])
        }
        result = {
            ...
            "ignored_out_of_scope_findings": 0,
            "ignored_out_of_scope_cwes": [],
            "repair": [],
            "expected_findings": [
                {
                    "path": str(item["path"]),
        ...
        try:
            ...
            all_findings = reviewer.review(case["diff"], parsed)
            findings = all_findings
            if scoring_scope == "target-cwe":
                findings = [
                    finding for finding in all_findings
                    if RULE_TO_CWE.get(finding.rule_id, finding.rule_id).upper()
                    in target_cwes
                ]
            result["ignored_out_of_scope_findings"] = len(all_findings) - len(findings)
            result["ignored_out_of_scope_cwes"] = sorted({
                RULE_TO_CWE.get(finding.rule_id, finding.rule_id).upper()
                for finding in all_findings if finding not in findings
            })
            ...
            matches = one_to_one_match(
                expected, findings, self.line_tolerance, scoring_scope
            )
        ...
        usage_after = read_evaluation_usage(reviewer)
        context_provider = getattr(reviewer, "evaluation_context", None)
        try:
            context = dict(context_provider() or {}) if callable(context_provider) else {}
        except Exception:
            context = {}
        result["context"] = context or {"status": "unavailable"}
        result["latency_ms"] = round((time.monotonic() - case_started) * 1000, 4)
        result["usage"] = usage_delta(usage_before, usage_after)
        return result

    # Compatibility for integrations that called the pre-0.8 private method.
    def _run_case(self, reviewer: Reviewer, case: dict) -> Dict[str, Any]:
        return self.run_case(reviewer, case)
...
def comparison_summary(
    ...
    quantitative_gates = {
        "dataset_parity": {
            ...
            "baseline_sha256": baseline["dataset"]["sha256"],
            "candidate_sha256": candidate["dataset"]["sha256"],
        },
    ...
    shared_policy = EvaluationPolicy(
        minimum_quality_improvement=minimum_f1_improvement,
    ).evaluate(
        baseline["metrics"], candidate["metrics"], improvement_metric="f1",
        protected_quality_metrics=(),
    )
    for gate_name in ("latency_p95", "total_tokens", "estimated_cost"):
        quantitative_gates[gate_name] = shared_policy["gates"][gate_name]
    source_kinds = set(candidate["dataset"].get("source_kinds") or [])
    integrity = candidate["dataset"].get("integrity") or {}
    trusted_source_kinds = {PUBLIC_SOURCE_KIND, BENCHMARK_SOURCE_KIND}
    provenance_gate = {
        "passed": (
            bool(source_kinds)
            and source_kinds.issubset(trusted_source_kinds)
            and integrity.get("valid") is True
        ),
        "allowed_source_kinds": sorted(trusted_source_kinds),
        "actual_source_kinds": sorted(source_kinds),
        "dataset_integrity_valid": integrity.get("valid", False),
        "integrity_errors": list(integrity.get("errors") or []),
    }
    ...
    return {
        ...
        "release_gate": {
            ...
            "resource_observability": shared_policy["observability"],
        },
    }

...
from codeevo.evolution import EvolutionEngine, RegressionEvaluator
from codeevo.fixer import SafeFixer
...
class AdvancedFeatureTests(unittest.TestCase):
    ...
    def test_explicit_local_provider_ignores_stale_custom_fields(self):
        local = settings(self.path)
        ...
        self.assertEqual({}, local.resolved_llm())

    def test_empty_provider_keeps_legacy_custom_auto_detection(self):
        custom = settings(self.path)
        ...
        self.assertEqual("custom", custom.resolved_llm()["provider"])

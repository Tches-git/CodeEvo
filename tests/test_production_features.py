...
from codeevo.harness import ReviewHarness
from codeevo.reviewer import LocalRuleReviewer
...
from codeevo.verifier import RepairVerifier
...
class ProductionFeatureTests(unittest.TestCase):
    ...
    def test_shadow_or_canary_requires_passing_offline_evaluation(self):
        release = ReleaseManager(self.store)
        with self.assertRaisesRegex(ValueError, "offline_evaluation"):
            release.configure("tenant", "skill", {
                "candidate_version": 2, "shadow_percent": 100,
            })
        failed = passing_offline_evaluation()
        failed["candidate"]["resource_usage"]["total_tokens"] = 2000
        with self.assertRaisesRegex(ValueError, "failed"):
            release.configure("tenant", "skill", {
                "candidate_version": 2, "canary_percent": 10,
                "offline_evaluation": failed,
            })

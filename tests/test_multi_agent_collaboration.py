...
from codeevo.reviewer import ReliabilityRuleReviewer, SecurityRuleReviewer
...
class MultiAgentCollaborationTests(unittest.TestCase):
    ...
    def test_agent_loop_can_be_disabled_for_offline_benchmark_review(self):
        class OneShotSpecialist:
            ...
            def review(self, _diff, parsed):
                ...
                line = parsed.added_lines[0]
                return [Finding(
                    "LLM-CORRECTNESS", Severity.MEDIUM, "Dynamic execution risk",
                    "The changed line executes untrusted input without validation.",
                    line.path, line.line, line.content,
                    "Replace dynamic execution with an allow-listed parser.",
                    "Add a malicious expression regression test.", 0.9,
                )]

            def agent_step(self, _state):
                self.loop_calls += 1
                raise AssertionError("agent loop must stay disabled")

        specialist = OneShotSpecialist()
        coordinator = MultiAgentCoordinator(
            [specialist], agent_loop_enabled=False, collaboration_rounds=1,
        )
        findings = coordinator.review(DIFF, parse_unified_diff(DIFF))
        self.assertEqual(1, len(findings))
        self.assertEqual(1, specialist.review_calls)
        self.assertEqual(0, specialist.loop_calls)

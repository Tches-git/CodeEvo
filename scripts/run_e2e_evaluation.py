...
from codeevo.evaluation_benchmark import (  # noqa: E402
    baseline_reviewer,
    candidate_reviewer,
    generate_controlled_pr_cases,
)
from codeevo.evaluation_harness import (  # noqa: E402
...
def markdown_report(baseline, candidate, comparison):
    ...
    lines = [
        "# CodeEvo 端到端 Evaluation Harness 报告",
        "",
        "## 数据集",
        "",
    ...
    lines.extend([
        ...
        "|---|---:|---:|",
        "| 平均延迟 | %.3f ms | %.3f ms |" % (
            b["resource_usage"]["latency_ms_mean"],
            c["resource_usage"]["latency_ms_mean"],
        ),
        "| P95 延迟 | %.3f ms | %.3f ms |" % (
            b["resource_usage"]["latency_ms_p95"],
            c["resource_usage"]["latency_ms_p95"],
        ),
        "| Model calls | %s | %s |" % (
            b["resource_usage"]["model_calls"], c["resource_usage"]["model_calls"],
        ),
        "| Total tokens | %s | %s |" % (
            b["resource_usage"]["total_tokens"], c["resource_usage"]["total_tokens"],
        ),
        "| 估算成本 | %s | %s |" % (
            b["resource_usage"]["estimated_cost_usd"],
            c["resource_usage"]["estimated_cost_usd"],
        ),
        "",
        "本基准使用确定性本地 Agent，因此模型调用、Token 和成本为 0（not applicable）；"
        "连接模型后若供应商不返回 usage，报告会明确标为 unavailable，不会估造数值。",
        "",
        "## 分区结果",
        "",
